...
export async function callModel(options: ModelCallOptions): Promise<ModelCallResult> {
  ...
  for (const provider of providers) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES_PER_MODEL; attempt++) {
      try {
        const result = await callSingleProvider(provider, options, attempt)
        return result
      } catch (error) {
  ...
  return new ReadableStream({
    async pull(controller) {
      while (true) {
        ...
        if (done) {
          controller.enqueue(encoder.encode(`data: {"chunk":"","done":true,"model":"${provider.model}","provider":"${provider.name}"}\n\n`))
          controller.close()
          return
        }
